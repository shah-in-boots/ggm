---
title: "The ggm style lab"
subtitle: >
  A live, tweakable style tile for the drawn-on-paper look —
  every color, font, and motion on this page is defined on this page.
format:
  html:
    toc: true
    toc-depth: 2
...
This page is the atomic workspace for ggm's visual identity. It is a
...
1. Open this `.qmd` next to a terminal and run
   `quarto preview ggm-style-lab.qmd`. The page re-renders on save.
...
already is). And this document is meant for the website, not the
package build: place it at `vignettes/articles/ggm-style-lab.qmd`,
which pkgdown renders but CRAN never sees
(`usethis::use_article("ggm-style-lab")` sets up the folder and the
...
The palette follows a drafting-room convention that suits ggm's
...
```{css}
/*-- ggm design tokens ------------------------------------*/
/*-- destined for: inst/assets/ggm-tokens.css -------------*/
...
```{js}
//-- ggm motion vocabulary --------------------------------
//-- destined for: inst/htmlwidgets/lib/ggm-motion.js -----
...
This is the moment from the anime.js homepage, rebuilt with ggm's own
...
<div class="gg-sheet">
  <svg id="ggSchematic" viewBox="0 0 640 300"
    ...
    <text class="gg-label" x="624" y="284" text-anchor="end"
          opacity="0">ggm — sheet 01 / 60 s @ 256 hz</text>
  </svg>
</div>
...
Bootstrap defaults. These panels are three plain `div`s using ggm's
...
| chunk on this page | destination in ggm |
|---|---|
| design tokens (CSS) | `inst/assets/ggm-tokens.css`, listed in the widget's YAML deps and the app skin |
| sheet + chrome + buttons (CSS) | the Shiny `htmlTemplate()` skin — Shiny keeps the reactive server, loses the Bootstrap look |
| motion vocabulary (JS) | `inst/htmlwidgets/lib/ggm-motion.js`, loaded before `ggm-anim.js`; `ggDrawIn()` and friends become the primitives directives call |
| schematic timeline (JS) | the target shape that `fx_seq()` / `fx_par()` specs compile to inside `ggm-anim.js` |
