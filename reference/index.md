# Package index

## All functions

- [`add_arrow()`](https://shah-in-boots.github.io/gram/reference/add_arrow.md)
  : Draw an arrow between two points
- [`at()`](https://shah-in-boots.github.io/gram/reference/at.md) : Refer
  to a point on a channel
- [`build_study_cache()`](https://shah-in-boots.github.io/gram/reference/build_study_cache.md)
  : Build the derived overview cache for a WFDB study
- [`cache_annotators()`](https://shah-in-boots.github.io/gram/reference/cache_annotators.md)
  : Available WFDB annotator sidecars
- [`cache_is_built()`](https://shah-in-boots.github.io/gram/reference/cache_is_built.md)
  : Has the overview cache been built for this study?
- [`cache_paths()`](https://shah-in-boots.github.io/gram/reference/cache_paths.md)
  : All paths in a study group
- [`cache_study_data()`](https://shah-in-boots.github.io/gram/reference/cache_study_data.md)
  : Download an example study to the local cache
- [`emphasize()`](https://shah-in-boots.github.io/gram/reference/emphasize.md)
  : Mark a point for emphasis
- [`eval_tracing()`](https://shah-in-boots.github.io/gram/reference/eval_tracing.md)
  : Evaluate a tracing script
- [`gram_harness()`](https://shah-in-boots.github.io/gram/reference/gram_harness.md)
  : Launch the development harness
- [`gram_plot()`](https://shah-in-boots.github.io/gram/reference/gram_plot.md)
  : Create a uPlot-backed electrogram widget
- [`gram_plotOutput()`](https://shah-in-boots.github.io/gram/reference/gram_plotOutput.md)
  : Shiny output binding for a gram plot
- [`gram_tracing()`](https://shah-in-boots.github.io/gram/reference/gram_tracing.md)
  : Render a tracing as an animated SVG widget
- [`gram_tracingOutput()`](https://shah-in-boots.github.io/gram/reference/gram_tracingOutput.md)
  : Shiny output binding for a tracing
- [`gram_verbs()`](https://shah-in-boots.github.io/gram/reference/gram_verbs.md)
  : Build the sealed environment a tracing script runs in
- [`read_cache_overview()`](https://shah-in-boots.github.io/gram/reference/read_cache_overview.md)
  : Read an overview cache window
- [`read_study_signal()`](https://shah-in-boots.github.io/gram/reference/read_study_signal.md)
  : Read raw WFDB signal for a visible window
- [`read_study_viewport()`](https://shah-in-boots.github.io/gram/reference/read_study_viewport.md)
  : Read the appropriate data for a viewport
- [`render_gram_plot()`](https://shah-in-boots.github.io/gram/reference/render_gram_plot.md)
  : Shiny render function for a gram plot
- [`render_gram_tracing()`](https://shah-in-boots.github.io/gram/reference/render_gram_tracing.md)
  : Shiny render function for a tracing
- [`reveal()`](https://shah-in-boots.github.io/gram/reference/reveal.md)
  : Draw the traces on, like pen on paper
- [`select_cache_level()`](https://shah-in-boots.github.io/gram/reference/select_cache_level.md)
  : Pick the best source level for a viewport
- [`study_cache()`](https://shah-in-boots.github.io/gram/reference/study_cache.md)
  : Open a WFDB record as a StudyCache handle
- [`sweep_window_seconds()`](https://shah-in-boots.github.io/gram/reference/sweep_window_seconds.md)
  : Visible window implied by a sweep speed
- [`tracing()`](https://shah-in-boots.github.io/gram/reference/tracing.md)
  : Open a signal window as a tracing
- [`tracing_spec()`](https://shah-in-boots.github.io/gram/reference/tracing_spec.md)
  : Compile a tracing to SVG and a timeline spec
- [`view_uplot()`](https://shah-in-boots.github.io/gram/reference/view_uplot.md)
  : View a study window with the uPlot backend
